...
def extract_card(img, output_fn=None, min_focus=120, debug=False):
    ...
    # We suppose that the contour with the largest area corresponds to the contour delimiting the card
...
def extract_cards_from_video(video_fn, output_dir=None, keep_ratio=5, min_focus=80, debug=False):
    """
...
def findHull(img, corner=refCornerHL, debug="no"):
    ...
    # Find the contours
    contours = cv2.findContours(thld.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
